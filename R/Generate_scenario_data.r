...
Generate_scenario_data <- function(Sim_Settings, seed = 123){
  ...
  ## Calculate the initial population distribution
	...
	map_grid <- expand.grid(X=Range_X, Y=Range_Y)
	...
	image.plot(Range_X, Range_Y, matrix(data.bathym$depth,nrow=length(Range_X), ncol=length(Range_Y)))
	...
	for (iyear in 1:n_years){
		...
		for (xyz in which(Effort_area_year[[iyear]]>0)){
			...
			if(do.tweedie==TRUE) rand.vessel <- matrix(sapply(1:n_species, function(x) rtweedie(Effort_area_year[[iyear]][xyz], mu= qq_vessel[which.vessel.fished], power=xi[x], phi=phi[x])),ncol=n_species)
  ...
  if(plotting==TRUE) {
	...
	Return = list(Data= Raw_data, Biomass=Bio, bathym=data.bathym)
	return(Return)
}
