...
Type objective_function<Type>::operator() ()
{
  ...
  for (int j=0; j<Nspecies; j++) {
    max_val += max(vector<Type>(yobs.col(j)));
  }
  ...
  PARAMETER_VECTOR(logTauE);                  // Precision parameter for the spatio-temporal variation for Nspecies
	...
	PARAMETER_VECTOR(Species_target);           // Product of cathability and species price (idea of targeting)
  ...
  // ======================== Calculate the linear predictors (link space) then back-transform for estimating the underlying abundance ========================
  ...
  // Step 3: Now add the contribution of spatial effect if existing using a spatial DFA approach or NOT
    ...
    // Spatial DFA approach = estimate for k latent field variables instead of the species
      if (do_spatialDFA == 1) {
  ...
  // ======================== Now modeling the catch process ========================

  matrix<Type> catcha(Nobs, Nspecies);
  // Step 1: Estimate the catchability effect:
  if (nobs_RE(0) >0){
    for (int i = 0; i < Nobs; i++){
      for (int j = 0; j < Nspecies; j++){
        int temp = 0;
        for (int k = 0; k < n_RE; k++) {
          if (k == 0) catcha(i,j) += RE(RE_indexes(i, k),j); // record it
          if (k > 0) {
            temp += nobs_RE(k-1);
            catcha(i,j) += RE(RE_indexes(i, k) + temp, j); // record it
          }
        }
      }
    }
    ...
    // Step 1: Modeling of the catch process
  vector<Type> Value_catch(Nobs);
  matrix<Type> Catch_pred(Nobs, Nspecies);
  Catch_pred.setZero();
  Value_catch.setZero();
  for (int i = 0; i < Nobs; i++){
    for (int j = 0; j < Nspecies; j++){
  ...
  // Now add the likelihood contribution of the spatial random effects
   ...
   // The catch process likelihood: Only tweedie at the moment
    ...
    for (int i=0; i<Nobs; i++) {
      for (int j = 0; j < Nspecies; j++){
        ...
        if (!isNA(yobs(i,j))) NLL -= dtweedie(yobs(i,j), Catch_pred(i,j), phi(j), s1, true);
        if (sim == 1) {
          SIMULATE { yobs(i,j) = rtweedie(Catch_pred(i,j), phi(j), s1);}
        }   // break;
      }
    }

    // The process of choosing the fishing site (binomial probability with P related to the value of the fishing location)
    for (int i=0; i<Nobs; i++) {
      NLL -= dbinom(Type(1), Type(1), Value_catch(i)/max_val, true);
    }
