...
// Function to creating the loading matrix of Nspecies to Nlatent factors
template<class Type>
...
Type objective_function<Type>::operator() ()
{
  ...
  // Step 3: Now add the contribution of spatial effect if existing (using the DFA approach)

    // Step 3.0: Define the loading matrix of the Nfactor to the Nspecies. Needed for the below-calculate spatial effects
      matrix<Type> Lspatial = create_loadings_covariance(L_val_spatial, Nspecies, Nfactor);

    // Step 3.1: the mean spatial field
      matrix<Type> omega_A(Nobs, Nlatent);
      for(int j=0;j<Nspecies; j++){
    ...
    if (include_st > 0) {
      ...
      array<Type> epsilon_st_A(Ast.rows(), Nyear, Nlatent);
      ...
      matrix<Type> epsilon_st_A_mat(Nobs, Nlatent);
      for (int i = 0; i < Nobs; i++){
        for (int j = 0; j < Nlatent; j++){
    ...
    } // end of spatio-temporal condition statement
 ...
 // The observation likelihood: Only tweedie at the moment
  ...
  // Adding the penalty term related to the latent factor approach
